...
public class GraphicsApi {
    ...
    @GetMapping("/sex/{dateBpa}/{cacheId}")
    public ResponseEntity<Object> getForSex(@PathVariable String dateBpa, @PathVariable String cacheId, Authentication authentication) {
        ...
        if(bpaOptional.isPresent()) {
            ...
            SexGraphicsDTO sexGraphicsDTO = graphicsService.dataSex(bpa, cacheId);
    ...
    @GetMapping("/sex/all/{cacheId}")
    public ResponseEntity<Object> getForSexAll(@PathVariable String cacheId, Authentication authentication) {
        User user = userService.userLogged(authentication);
        ...
        if(!bpaList.isEmpty()) {
            ...
            SexGraphicsDTO sexGraphicsDTO = graphicsService.dataSexAll(bpaList, cacheId);

            return ResponseEntity.ok(sexGraphicsDTO);
        }

        return ResponseEntity.badRequest().body("NOT FOUND FILES BPA");
    }
